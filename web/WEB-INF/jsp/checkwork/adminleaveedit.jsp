...
$(function(){
  ...
  <body>
    <div class="x-body">
        ...
        <!-- id为员工信息登录名-->
        <input type="hidden" name="id" id="leave_id" value="${leave.id}" >
              ...
              <div class="layui-input-inline">
                  <select id="leavetype" name="leavetype" class="valid">
                    <c:forEach items="${requestScope.leavetype_list}" var="job">
						   		<c:choose>
			    					<c:when test="${leave.leavetype == job.id }">
              ...
              <div class="layui-input-inline">
                  <select id="leavestatus" name="leavestatus" class="valid">
                    <c:forEach items="${requestScope.leavestatus_list}" var="job">
						   		<c:choose>
			    					<c:when test="${leave.leavestatus == job.id }">
			    						<option value="${job.id }" selected="selected">${job.name }</option>
			    					</c:when>
			    					<c:otherwise>
			    						<option value="${job.id }">${job.name}</option>
  ...
  <script>
      $("#ret").click(function () {
          open("/checkwork/adminleavelist","_self")
      })
  </script>
  </body>
